...
void show() {
  for (uint16_t i=0; i < remappedStrip.numPixels(); i++) {
    if(i<150) {
      remappedStrip.setPixelColor(i, strip.getPixelColor(i));    //turn every third pixel on
    }
    else {
      remappedStrip.setPixelColor(i, strip.getPixelColor(i+1));    //turn every third pixel on
    }
  }
  remappedStrip.show();
}
